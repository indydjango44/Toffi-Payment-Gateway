...
class Toffi_onramp_custom_endpoint
{
    ...
    function permissions_onramp_check($request)
    {
        ...
        if (empty($provided) && isset($_SERVER['HTTP_X_WEBSITE_KEY'])) {
            $provided = $_SERVER['HTTP_X_WEBSITE_KEY'];
        }

        if (empty($provided) && function_exists('getallheaders')) {
            $headers = array_change_key_case(getallheaders(), CASE_LOWER);
            if (isset($headers['x-website-key'])) {
                $provided = $headers['x-website-key'];
            }
        }

        // 3) Compare with your expected value (must match what your server generates)
        $expected = create_hash(); // make sure this uses UTC if your .NET side uses UTC
